...
def negSamplingLossAndGradient(
    ...
    # Calculate sigmoid for the positive sample
    z = np.dot(outsideVectors[outsideWordIdx], centerWordVec)
    pos_sigmoid = sigmoid(z)
    loss -= np.log(pos_sigmoid)
    gradCenterVec += (pos_sigmoid - 1.0) * outsideVectors[outsideWordIdx]
    gradOutsideVecs[outsideWordIdx] += (pos_sigmoid - 1.0) * centerWordVec

    # Calculate sigmoid for the negative samples
    for negIdx in negSampleWordIndices:
        z_neg = np.dot(outsideVectors[negIdx], centerWordVec)
        neg_sigmoid = sigmoid(-z_neg)
        loss -= np.log(neg_sigmoid)
        gradCenterVec += (1.0 - neg_sigmoid) * outsideVectors[negIdx]
        gradOutsideVecs[negIdx] += (1.0 - neg_sigmoid) * centerWordVec
    ### END YOUR CODE
